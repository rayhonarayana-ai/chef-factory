...
export interface WorkforceOrchestratorOptions {
  ...
  /** Gate 46 — completion-boundary workspace guard closing the final TOCTOU interval. */
...
export async function runWorkforce(opts: WorkforceOrchestratorOptions): Promise<WorkforceOrchestratorResult> {
  ...
  for (let i = 0; i < placedTasks.length; i += maxParallelExecutions) {
    ...
    const results = await Promise.all(
      slice.map((p) =>
        executeAssignedAgentTask({ store, execution, ownerId, agentId: p.agentId, taskId: p.taskId, verification, completionWorkspaceGuard })
